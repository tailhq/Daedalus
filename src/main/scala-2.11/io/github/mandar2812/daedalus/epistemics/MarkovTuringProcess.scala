...
/**
  ...
  */
class MarkovTuringProcess(
  num_states: Int,
  conditional: ((Int, Int)) => DenseVector[Double],
  cellStates: Int = 2)
  ...
  val encoding = TupleIntegerEncoding(List(2, 3, num_states))

  def _num_states = num_states

  override def run(data: (Int, Int)): RandomVariable[(Int, Int, Int)] =
    MeasurableFunction[Int, (Int, Int, Int), DiscreteDistrRV[Int]](
      RandomVariable(new Multinomial(conditional(data))))(
      (n: Int) => {
        val l = encoding.i(n)
        (l.head, l(1), l(2))
      })
